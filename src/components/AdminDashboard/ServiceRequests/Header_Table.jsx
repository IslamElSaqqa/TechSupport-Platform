...
const Header_Table = ({  title = "Service Requests", profileName = "Admin"  }) => {
  ...
  useEffect(() => { fetchRequests(); }, [currentPage, searchTerm, sortOrder, shopsTypeFilter]);
  ...
  try {
    ...
    const header = ['User ID', 'User Name', 'User phone', 'Specialist ID', 'Specialist Name', 'Specialization', 'Specialist email', 'Specialist Phone', 'id', 'status', 'instaID', 'note', 'description', 'image_URL', 'steps', 'createdAt', 'UpdatedAt'];
              ...
              {requests.map(request => (
                <tr key={request._id}>
                  <td className={styles.checkboxCell}>
                    <input type="checkbox" checked={selectedItems.includes(request._id)} onChange={() => handleSelectItem(request._id)} />
                  </td>
                  <td>{request.specialist ? request.specialist.specialist_name : ""}</td>
                  <td>{request.createdAt}</td>
                  <td>{request._id}</td>
                  <td>{request.user ? request.user.username : ""}</td>
                  <td>{request.user ? request.user.phone : ""}</td>
                  <td><span onClick={() => handleDetailsClick(request)} style={{ cursor: 'pointer' }}>{request.status}...</span></td>
                  <td>{request.instapay_reference}</td>
                  <td>
                    <FaEdit className={styles.editIcon} onClick={() => openEditForm(request)} />
                  </td>
                </tr>
              ))}
            </tbody>
